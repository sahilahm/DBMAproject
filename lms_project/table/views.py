...
from .models import (users_table, login_time, num_of_leaves, cur_running,
...
def application(request, pro):
    if request.method =='POST':
        ...
        ok = pro
        f = users_table.objects.get(id = pro)
        if f.dept_name == 'CSE' and f.pos =='faculty': 
            try :
                user =  cse_dept.objects.get(pro_id = ok)
            except cse_dept.DoesNotExist:
                user = None
            if user is None :
        ...
        if f.dept_name == 'EE'and f.pos =='faculty':
            try :
                user =  ee_dept.objects.get(pro_id = ok)
            except ee_dept.DoesNotExist:
                user = None
            if user is None  :
                return render(request,"application.html",{'data':pro})
            else:
        ...
        if f.dept_name == 'MEC'and f.pos =='faculty':
            try :
                user =  mec_dept.objects.get(pro_id = ok)
            except mec_dept.DoesNotExist:
                user = None
            if user is None  :
                return render(request,"application.html",{'data':pro})
            else:
...
def changecsehod(request,pro):
        if request.method == 'POST':
            ...
            if pos =='CSEHOD':
                users_table.objects.filter(pos ='CSEHOD').update(pos ='faculty')
                users_table.objects.filter(id = pro_id).update(pos ='CSEHOD')
                return HttpResponse("done")
            else:
                users_table.objects.filter(pos ='Dean').update(pos ='faculty')
                users_table.objects.filter(id = pro_id).update(pos ='Dean')
                return HttpResponse("done")
        else:
            try :
                p = users_table.objects.filter(dept_name ='CSE')
...
def changeeehod(request,pro):
        if request.method == 'POST':
            ...
            pos = request.POST['status']
            if pos =='EEHOD':
                users_table.objects.filter(pos ='EEHOD').update(pos ='faculty')
                users_table.objects.filter(id = pro_id).update(pos ='EEHOD')
                return HttpResponse("done")
            else:
                users_table.objects.filter(pos ='Dean').update(pos ='faculty')
                users_table.objects.filter(id = pro_id).update(pos ='Dean')
            ...
            try :
                p = users_table.objects.filter(dept_name ='EE')
            except users_table.DoesNotExist:
                p = None
            if p is None:
...
def changemechod(request,pro):
        if request.method == 'POST':
            ...
            pos = request.POST['status']
            if pos =='MECHOD':
                users_table.objects.filter(pos ='MECHOD').update(pos ='faculty')
                users_table.objects.filter(id = pro_id).update(pos ='MECHOD')
                return HttpResponse("done")
            else:
                users_table.objects.filter(pos ='Dean').update(pos ='faculty')
                users_table.objects.filter(id = pro_id).update(pos ='Dean')
            ...
            try :
                p = users_table.objects.filter(dept_name ='MEC')
            except users_table.DoesNotExist:
                p = None
            if p is None:
